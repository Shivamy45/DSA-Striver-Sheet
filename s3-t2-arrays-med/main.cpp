...
void twoSum2(vector<int>& nums, int target) {
    ...
    for (int i = 0; i < nums.size(); i++) {
        ...
        if (it != mpp.end()) {
            cout << "arr[" << mpp[target - nums[i]] << "] + arr[" << i
...
void majorityElement(vector<int>& nums) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (count > n / 2) {
            cout << nums[i] << " came " << count << " times in the array."
                 << endl;
            return;
        }
    }
...
void majorityElement2(vector<int>& nums) {
    ...
    for (auto it : mpp) {
        if (it.second > n / 2) {
            cout << it.first << " came " << it.second << " times in the array."
                 << endl;
            return;
        }
    }
...
void maxSumOfSubArray(vector<int>& nums) {
    ...
    cout << "Max sum " << res << " starts from index " << ansStart << " to "
         << ansEnd << endl;
}

//! This only works for +ve MaxSum of SubArray
void maxSumOfSubArray2(vector<int>& nums) {
    int n = nums.size(), sum = 0, res = INT_MIN, start = -1, ansStart = -1,
        ansEnd = -1;
    for (int i = 0; i < n; i++) {
        if (sum == 0) start = i;
