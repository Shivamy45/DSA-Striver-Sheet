...
bool helperExist(vector<vector<char>>& board, string& word, int i, int j,
    ...
    board[i][j] = '#';
    bool found = helperExist(board, word, i + 1, j, k + 1) ||
                 helperExist(board, word, i, j + 1, k + 1) ||
                 helperExist(board, word, i - 1, j, k + 1) ||
                 helperExist(board, word, i, j - 1, k + 1);
    board[i][j] = saved;
    return found;
}
...
bool exist(vector<vector<char>>& board, string word) {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            if (helperExist(board, word, i, j, 0)) return true;
    return false;
}
...
bool isSafe(vector<string>& board, int row, int col) {
    ...
    // left
    while (colCopy >= 0) {
        if (board[row][colCopy] == 'Q') return false;
        colCopy--;
    }
    // upper diag
    colCopy = col;
    while (rowCopy >= 0 && colCopy >= 0) {
        if (board[rowCopy][colCopy] == 'Q') return false;
        rowCopy--;
        colCopy--;
    }
    // lower diag
...
void helperSolveNQueens(vector<vector<string>>& res, vector<string>& board,
    ...
    for (int row = 0; row < n; row++) {
        if (!(upperDiag[n - 1 + col - row] || lowerDiag[row + col] ||
              left[row])) {
            left[row] = lowerDiag[row + col] = upperDiag[n - 1 + col - row] =
                true;
            board[row][col] = 'Q';
            helperSolveNQueens(res, board, col + 1, n, left, lowerDiag,
                               upperDiag);
            board[row][col] = '.';
            upperDiag[n - 1 + col - row] = lowerDiag[row + col] = left[row] =
                false;
        }
    }
}
...
void helperRatInMaze(vector<vector<int>>& maze, vector<string>& res,
                     string& path, int row, int col, int n, int m,
...
bool isPossible(int n, vector<vector<int>>& graph, vector<int>& colored,
                int node) {
    for (int i = 0; i < n; i++)
...
bool helperPossibleMColoring(int n, int m, vector<vector<int>>& graph,
                             vector<int>& colored, int node) {
...
bool helperSolveSodoku(vector<vector<char>>& board, int row, int col) {
    ...
    for (char c = '1'; c <= '9'; c++) {
        if (isPossible2(board, row, col, c)) {
            board[row][col] = c;
            if (helperSolveSodoku(board, row, col + 1)) return true;
            board[row][col] = '.';
        }
    }
...
void helperAddOperators(string num, int target, int idx, long long curr,
                        long long prevOperand, string temp,
                        vector<string>& res) {
    if (idx == num.length()) {
    ...
    for (int i = idx; i < num.length(); i++) {
        ...
        if (idx == 0) {
            helperAddOperators(num, target, i + 1, val, val, valString, res);
        } else {
            ...
            helperAddOperators(num, target, i + 1,
                               ...
                               prevOperand * val, temp + '*' + valString, res);
        }
    }
}
...
int main() {
    ...
    for (string& a : wordDict) getline(cin, a);
    ...
    for (auto& it : edges) cin >> it.first >> it.second;
